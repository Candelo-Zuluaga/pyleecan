...
class SlotM11(Slot):
    ...
    if isinstance(is_airgap_active, ImportError):
        is_airgap_active = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotM11 method is_airgap_active: " + str(is_airgap_active)
                )
            )
        )
    else:
    ...
    def __init__(self, W0=0, H0=0, W1=0, H1=0, Zs=36, wedge_mat=None, is_bore=True, init_dict = None, init_str = None):
