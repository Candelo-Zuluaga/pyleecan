...
class SlotM18(Slot):
    ...
    if isinstance(is_airgap_active, ImportError):
        is_airgap_active = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use SlotM18 method is_airgap_active: " + str(is_airgap_active)
                )
            )
        )
    else:
    ...
    def __init__(self, H0=0.001, Zs=36, wedge_mat=None, is_bore=True, init_dict = None, init_str = None):
    ...
    def copy(self):
        ...
        obj_copy = type(self)(H0=H0_val,Zs=Zs_val,wedge_mat=wedge_mat_val,is_bore=is_bore_val)
        return obj_copy
