...
def comp_wind_function(self, angle=None, Na=2048, alpha_mmf0=0, per_a=1):
    ...
    for ii in range(0,len(self.slot_list)):
        if self.type_list[ii]=='W':
            Zs = self.slot_list[ii].Zs  # Number of slot
            Zs0 = int(Zs / per_a)
            slot_angle = self.slot_list[ii].comp_angle_active_eq()
            slot_opening = self.slot_list[ii].comp_angle_opening()
